...
class RDShaderFile extends gd.Resource {
	...
	public function set_bytecode(p_bytecode:gd.RDShaderSPIRV, ?p_version:std.String = "&\"\""):Void __rdshaderfile_ptr().value.set_bytecode(p_bytecode, p_version);
	public function get_spirv(?p_version:std.String = "&\"\""):gd.RDShaderSPIRV return __rdshaderfile_ptr().value.get_spirv(p_version);
	public function set_base_error(p_error:std.String):std.String {
		__rdshaderfile_ptr().value.set_base_error(p_error);
		return p_error;
	}
