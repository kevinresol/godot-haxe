...
class PacketPeer extends gd.RefCounted {
	...
	public function get_var(?p_allow_objects:Bool = false):gd.Variant return __packetpeer_ptr().value.get_var(p_allow_objects);
	public function put_var(p_var:gd.Variant, ?p_full_objects:Bool = false):gd.Error return __packetpeer_ptr().value.put_var(p_var, p_full_objects);
	public function get_packet():gd.PackedByteArray return __packetpeer_ptr().value.get_packet();
	public function put_packet(p_buffer:gd.PackedByteArray):gd.Error return __packetpeer_ptr().value.put_packet(p_buffer);
	...
	public function set_encode_buffer_max_size(p_max_size:Int):Int {
		__packetpeer_ptr().value.set_encode_buffer_max_size(p_max_size);
		return p_max_size;
	}
