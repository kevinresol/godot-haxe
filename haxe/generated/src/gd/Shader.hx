...
class Shader extends gd.Resource {
	...
	public function set_code(p_code:std.String):std.String {
		__shader_ptr().value.set_code(p_code);
		return p_code;
	}
	public function get_code():std.String return __shader_ptr().value.get_code();
	public function set_default_texture_parameter(p_name:std.String, p_texture:gd.Texture2D, ?p_index:Int = 0):Void __shader_ptr().value.set_default_texture_parameter(p_name, p_texture, p_index);
	public function get_default_texture_parameter(p_name:std.String, ?p_index:Int = 0):gd.Texture2D return __shader_ptr().value.get_default_texture_parameter(p_name, p_index);
	public function get_shader_uniform_list(?p_get_groups:Bool = false):gd.Array return __shader_ptr().value.get_shader_uniform_list(p_get_groups);
