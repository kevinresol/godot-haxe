...
class PhysicsBody3D extends gd.CollisionObject3D {
	...
	public function move_and_collide(p_motion:gd.Vector3, ?p_test_only:Bool = false, ?p_safe_margin:Float = 0.001, ?p_recovery_as_collision:Bool = false, ?p_max_collisions:Int = 1):gd.KinematicCollision3D return __physicsbody3d_ptr().value.move_and_collide(p_motion, p_test_only, p_safe_margin, p_recovery_as_collision, p_max_collisions);
	public function get_gravity():gd.Vector3 return __physicsbody3d_ptr().value.get_gravity();
	public function set_axis_lock(p_axis:gd.physicsserver3d.BodyAxis, p_lock:Bool):Void __physicsbody3d_ptr().value.set_axis_lock(p_axis, p_lock);
	public function get_axis_lock(p_axis:gd.physicsserver3d.BodyAxis):Bool return __physicsbody3d_ptr().value.get_axis_lock(p_axis);
	public function add_collision_exception_with(p_body:gd.Node):Void __physicsbody3d_ptr().value.add_collision_exception_with(p_body);
	public function remove_collision_exception_with(p_body:gd.Node):Void __physicsbody3d_ptr().value.remove_collision_exception_with(p_body);
