...
class UDPServer extends gd.RefCounted {
	...
	public function listen(p_port:Int, ?p_bind_address:std.String = "\"*\""):gd.Error return __udpserver_ptr().value.listen(p_port, p_bind_address);
	...
	public function set_max_pending_connections(p_max_pending_connections:Int):Int {
		__udpserver_ptr().value.set_max_pending_connections(p_max_pending_connections);
