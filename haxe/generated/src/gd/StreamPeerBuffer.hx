...
class StreamPeerBuffer extends gd.StreamPeer {
	...
	public function seek(p_position:Int):Void __streampeerbuffer_ptr().value.seek(p_position);
	...
	public function resize(p_size:Int):Void __streampeerbuffer_ptr().value.resize(p_size);
	public function set_data_array(p_data:gd.PackedByteArray):gd.PackedByteArray {
		__streampeerbuffer_ptr().value.set_data_array(p_data);
		return p_data;
	}
