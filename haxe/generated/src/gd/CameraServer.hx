...
class CameraServer extends gd.Object {
	...
	public function get_feed(p_index:Int):gd.CameraFeed return __cameraserver_ptr().value.get_feed(p_index);
	public function get_feed_count():Int return __cameraserver_ptr().value.get_feed_count();
	public function add_feed(p_feed:gd.CameraFeed):Void __cameraserver_ptr().value.add_feed(p_feed);
	public function remove_feed(p_feed:gd.CameraFeed):Void __cameraserver_ptr().value.remove_feed(p_feed);
}