...
class DampedSpringJoint2D extends gd.Joint2D {
	...
	public function set_length(p_length:Float):Float {
		__dampedspringjoint2d_ptr().value.set_length(p_length);
		return p_length;
	}
	...
	public function set_rest_length(p_rest_length:Float):Float {
		__dampedspringjoint2d_ptr().value.set_rest_length(p_rest_length);
	...
	public function set_stiffness(p_stiffness:Float):Float {
		__dampedspringjoint2d_ptr().value.set_stiffness(p_stiffness);
		return p_stiffness;
	}
	...
	public function set_damping(p_damping:Float):Float {
		__dampedspringjoint2d_ptr().value.set_damping(p_damping);
		return p_damping;
	}
