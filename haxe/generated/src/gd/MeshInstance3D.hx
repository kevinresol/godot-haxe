...
class MeshInstance3D extends gd.GeometryInstance3D {
	...
	public function set_mesh(p_mesh:gd.Mesh):gd.Mesh {
		__meshinstance3d_ptr().value.set_mesh(p_mesh);
		return p_mesh;
	}
	public function get_mesh():gd.Mesh return __meshinstance3d_ptr().value.get_mesh();
	public function set_skeleton_path(p_skeleton_path:std.String):Void __meshinstance3d_ptr().value.set_skeleton_path(p_skeleton_path);
	...
	public function set_skin(p_skin:gd.Skin):gd.Skin {
		__meshinstance3d_ptr().value.set_skin(p_skin);
		return p_skin;
	}
	...
	public function set_surface_override_material(p_surface:Int, p_material:gd.Material):Void __meshinstance3d_ptr().value.set_surface_override_material(p_surface, p_material);
	public function get_surface_override_material(p_surface:Int):gd.Material return __meshinstance3d_ptr().value.get_surface_override_material(p_surface);
	public function get_active_material(p_surface:Int):gd.Material return __meshinstance3d_ptr().value.get_active_material(p_surface);
	public function create_trimesh_collision():Void __meshinstance3d_ptr().value.create_trimesh_collision();
	public function create_convex_collision(?p_clean:Bool = true, ?p_simplify:Bool = false):Void __meshinstance3d_ptr().value.create_convex_collision(p_clean, p_simplify);
	public function get_blend_shape_count():Int return __meshinstance3d_ptr().value.get_blend_shape_count();
	public function find_blend_shape_by_name(p_name:std.String):Int return __meshinstance3d_ptr().value.find_blend_shape_by_name(p_name);
	public function get_blend_shape_value(p_blend_shape_idx:Int):Float return __meshinstance3d_ptr().value.get_blend_shape_value(p_blend_shape_idx);
	public function set_blend_shape_value(p_blend_shape_idx:Int, p_value:Float):Void __meshinstance3d_ptr().value.set_blend_shape_value(p_blend_shape_idx, p_value);
