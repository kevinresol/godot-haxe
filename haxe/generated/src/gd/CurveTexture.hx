...
class CurveTexture extends gd.Texture2D {
	...
	public function set_width(p_width:Int):Void __curvetexture_ptr().value.set_width(p_width);
	public function set_curve(p_curve:gd.Curve):gd.Curve {
		__curvetexture_ptr().value.set_curve(p_curve);
		return p_curve;
	}
	...
	public function set_texture_mode(p_texture_mode:gd.curvetexture.TextureMode):gd.curvetexture.TextureMode {
		__curvetexture_ptr().value.set_texture_mode(p_texture_mode);
