...
enum abstract LightParam(Int) from Int to Int {
	@:op(A | B)
	extern static inline function or(a:LightParam, b:LightParam):LightParam {
		return untyped __cpp__('({0}) | ({1})', a, b);
	}
	@:to
	extern inline function toNative():gdnative.renderingserver.LightParam return untyped __cpp__("static_cast<godot::RenderingServer::LightParam>({0})", this);
	@:from
	extern static inline function fromNative(v:gdnative.renderingserver.LightParam):LightParam return untyped __cpp__("static_cast<int32_t>({0})", v);
	final ENERGY = 0;
