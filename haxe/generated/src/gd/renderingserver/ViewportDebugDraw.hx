...
enum abstract ViewportDebugDraw(Int) from Int to Int {
	@:op(A | B)
	extern static inline function or(a:ViewportDebugDraw, b:ViewportDebugDraw):ViewportDebugDraw {
		return untyped __cpp__('({0}) | ({1})', a, b);
	}
	@:to
	extern inline function toNative():gdnative.renderingserver.ViewportDebugDraw return untyped __cpp__("static_cast<godot::RenderingServer::ViewportDebugDraw>({0})", this);
	@:from
	extern static inline function fromNative(v:gdnative.renderingserver.ViewportDebugDraw):ViewportDebugDraw return untyped __cpp__("static_cast<int32_t>({0})", v);
	final DISABLED = 0;
	final UNSHADED = 1;
	final LIGHTING = 2;
	final OVERDRAW = 3;
