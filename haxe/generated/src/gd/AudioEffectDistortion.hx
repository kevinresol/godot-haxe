...
class AudioEffectDistortion extends gd.AudioEffect {
	...
	public function set_mode(p_mode:gd.audioeffectdistortion.Mode):gd.audioeffectdistortion.Mode {
		__audioeffectdistortion_ptr().value.set_mode(p_mode);
		return p_mode;
	}
	...
	public function set_pre_gain(p_pre_gain:Float):Float {
		__audioeffectdistortion_ptr().value.set_pre_gain(p_pre_gain);
		return p_pre_gain;
	}
	...
	public function set_keep_hf_hz(p_keep_hf_hz:Float):Float {
		__audioeffectdistortion_ptr().value.set_keep_hf_hz(p_keep_hf_hz);
		return p_keep_hf_hz;
	}
	...
	public function set_drive(p_drive:Float):Float {
		__audioeffectdistortion_ptr().value.set_drive(p_drive);
		return p_drive;
	}
	...
	public function set_post_gain(p_post_gain:Float):Float {
		__audioeffectdistortion_ptr().value.set_post_gain(p_post_gain);
		return p_post_gain;
	}
