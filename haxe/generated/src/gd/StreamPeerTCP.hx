...
class StreamPeerTCP extends gd.StreamPeer {
	...
	public function bind(p_port:Int, ?p_host:std.String = "\"*\""):gd.Error return __streampeertcp_ptr().value.bind(p_port, p_host);
	public function connect_to_host(p_host:std.String, p_port:Int):gd.Error return __streampeertcp_ptr().value.connect_to_host(p_host, p_port);
	...
	public function set_no_delay(p_enabled:Bool):Void __streampeertcp_ptr().value.set_no_delay(p_enabled);
}