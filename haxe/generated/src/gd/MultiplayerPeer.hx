...
class MultiplayerPeer extends gd.PacketPeer {
	...
	public function set_transfer_channel(p_channel:Int):Int {
		__multiplayerpeer_ptr().value.set_transfer_channel(p_channel);
		return p_channel;
	}
	...
	public function set_transfer_mode(p_mode:gd.multiplayerpeer.TransferMode):gd.multiplayerpeer.TransferMode {
		__multiplayerpeer_ptr().value.set_transfer_mode(p_mode);
		return p_mode;
	}
	public function get_transfer_mode():gd.multiplayerpeer.TransferMode return __multiplayerpeer_ptr().value.get_transfer_mode();
	public function set_target_peer(p_id:Int):Void __multiplayerpeer_ptr().value.set_target_peer(p_id);
	...
	public function disconnect_peer(p_peer:Int, ?p_force:Bool = false):Void __multiplayerpeer_ptr().value.disconnect_peer(p_peer, p_force);
	...
	public function set_refuse_new_connections(p_enable:Bool):Bool {
		__multiplayerpeer_ptr().value.set_refuse_new_connections(p_enable);
		return p_enable;
	}
