...
class SpringArm3D extends gd.Node3D {
	...
	public function set_length(p_length:Float):Void __springarm3d_ptr().value.set_length(p_length);
	...
	public function set_shape(p_shape:gd.Shape3D):gd.Shape3D {
		__springarm3d_ptr().value.set_shape(p_shape);
		return p_shape;
	}
	public function get_shape():gd.Shape3D return __springarm3d_ptr().value.get_shape();
	public function add_excluded_object(p_RID:gd.RID):Void __springarm3d_ptr().value.add_excluded_object(p_RID);
	public function remove_excluded_object(p_RID:gd.RID):Bool return __springarm3d_ptr().value.remove_excluded_object(p_RID);
	...
	public function set_collision_mask(p_mask:Int):Int {
		__springarm3d_ptr().value.set_collision_mask(p_mask);
		return p_mask;
	}
	...
	public function set_margin(p_margin:Float):Float {
		__springarm3d_ptr().value.set_margin(p_margin);
		return p_margin;
	}
