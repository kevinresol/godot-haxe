...
class XRVRS extends gd.Object {
	...
	public function set_vrs_min_radius(p_radius:Float):Float {
		__xrvrs_ptr().value.set_vrs_min_radius(p_radius);
		return p_radius;
	}
	...
	public function set_vrs_strength(p_strength:Float):Float {
		__xrvrs_ptr().value.set_vrs_strength(p_strength);
		return p_strength;
	}
	public function make_vrs_texture(p_target_size:gd.Vector2, p_eye_foci:gd.PackedVector2Array):gd.RID return __xrvrs_ptr().value.make_vrs_texture(p_target_size, p_eye_foci);
