...
class MovieWriter extends gd.Object {
	...
	public function _handles_file(p_path:std.String):Bool return __moviewriter_ptr().value._handles_file(p_path);
	public function _write_begin(p_movie_size:gd.Vector2i, p_fps:Int, p_base_path:std.String):gd.Error return __moviewriter_ptr().value._write_begin(p_movie_size, p_fps, p_base_path);
	public function _write_end():Void __moviewriter_ptr().value._write_end();
	public static function add_writer(p_writer:gd.MovieWriter):Void gdnative.MovieWriter.MovieWriter_extern.add_writer(p_writer);
}