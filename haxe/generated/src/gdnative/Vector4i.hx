...
@:forward abstract Vector4i(cpp.Struct<Vector4i_extern>) from cpp.Struct<Vector4i_extern> to cpp.Struct<Vector4i_extern> {
	...
	@:op(A * B)
	extern inline function __op_multiply_int(p_rhs:Int):gdnative.Vector4i return untyped __cpp__('{0} * {1}', val(), p_rhs);
	@:op(A % B)
	extern inline function __op_modulus_int(p_rhs:Int):gdnative.Vector4i return untyped __cpp__('{0} % {1}', val(), p_rhs);
	@:op(A == B)
